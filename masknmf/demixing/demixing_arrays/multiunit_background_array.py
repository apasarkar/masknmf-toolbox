...
class MultiunitBackgroundArray(ArrayLike):
    """
    ...
    @property
    def multiunit_basis_term2(self) -> torch.Tensor:
    ...
    def getitem_tensor(
        ...
        multiunit_basis_term2_crop = self.multiunit_basis_term2[:, frame_indexer]
        if multiunit_basis_term2_crop.ndim < self.multiunit_basis_term2.ndim:
            multiunit_basis_term2_crop = multiunit_basis_term2_crop.unsqueeze(1)
        ...
        if np.prod(implied_fov) <= multiunit_basis_term2_crop.shape[1]:
            product = torch.sparse.mm(u_crop, self.multiunit_basis_term1)
            product = torch.matmul(product, multiunit_basis_term2_crop)

        else:
            product = torch.matmul(self.multiunit_basis_term1, multiunit_basis_term2_crop)
