"""
Class label sets for the ROI viewers: the palette, the 1-9 hotkeys, and a
label vector that survives ROIs being added, removed and renumbered.
"""
...
# tab10, so a label set looks the same in every masknmf viewer
LABEL_COLORS = (
...
def class_color(index: int) -> tuple:
    """rgb in 0-1 for a class index, wrapping past the palette end"""
    return LABEL_COLORS[index % len(LABEL_COLORS)]


class LabelSet:
    """Class names plus one label per item; -1 is unlabeled."""
    ...
    def add(self, name: str) -> bool:
        """Append a class name; False when it is empty or already present."""
    ...
    def remove(self, index: int) -> bool:
        """Drop a class: its items become unlabeled, higher labels shift down."""
    ...
    def hotkey_pressed(self) -> Optional[int]:
        """Label index for a pressed 1-9 key, UNLABELED for 0, else None."""
