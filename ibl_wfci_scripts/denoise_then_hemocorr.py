...
def _run_pipeline(cfg: DictConfig) -> None:
    ...
    np.savez(os.path.join(out_save_dir, "pmd_no_nn_gcamp.npz"), pmd=pmd_gcamp_no_nn)

    display("Training NN for denoising the gcamp channel")
    v = pmd_gcamp_no_nn.v.cpu()
    trained_model, _ = masknmf.compression.denoising.train_total_variance_denoiser(v,
                                                                                   max_epochs=5,
                                                                                   batch_size=128,
                                                                                   learning_rate=1e-4)

    trained_gcamp_module = masknmf.compression.PMDTemporalDenoiser(trained_model)

    pmd_gcamp_with_nn = masknmf.compression.pmd_decomposition(gcamp_channel,
                                                            block_sizes,
                                                            gcamp_channel.shape[0],
                                                            max_components=20,
                                                            ...
                                                            spatial_avg_factor=1,
                                                            device='cuda',
                                                            temporal_denoiser=trained_gcamp_module,
                                                            frame_batch_size=1024,
                                                            pixel_weighting=mask)


    np.savez(os.path.join(out_save_dir, "pmd_gcamp_nn.npz"), pmd=pmd_gcamp_with_nn)
    ...
    np.savez(os.path.join(out_save_dir, "pmd_no_nn_hemo.npz"), pmd=pmd_hemo_no_nn)

    display("Training NN for denoising the hemo channel")
    v = pmd_hemo_no_nn.v.cpu()
    trained_model, _ = masknmf.compression.denoising.train_total_variance_denoiser(v,
                                                                                   max_epochs=5,
                                                                                   batch_size=128,
                                                                                   learning_rate=1e-4)

    trained_hemo_module = masknmf.compression.PMDTemporalDenoiser(trained_model)

    pmd_hemo_with_nn = masknmf.compression.pmd_decomposition(blood_channel,
                                                           block_sizes,
                                                           gcamp_channel.shape[0],
                                                           max_components=cfg.max_components,
                                                           max_consecutive_failures=cfg.max_consecutive_failures,
                                                           ...
                                                           spatial_avg_factor=1,
                                                           device='cuda',
                                                           temporal_denoiser=trained_hemo_module,
                                                           frame_batch_size=1024,
                                                           pixel_weighting=mask)

    np.savez(os.path.join(out_save_dir, "pmd_hemo_nn.npz"), pmd=pmd_hemo_with_nn)

    blood_indices = np.array([i * 2 + 1 for i in range(pmd_hemo_with_nn.shape[0])])
    gcamp_indices = np.array([i * 2 for i in range(pmd_gcamp_with_nn.shape[0])])

    t = np.arange(pmd_hemo_with_nn.shape[0] * 2 + 1)
    v_hemo_interp = interp1d(blood_indices,
                             pmd_hemo_with_nn.v.cpu().numpy(),
                             axis=1,
                             fill_value='extrapolate')(
        gcamp_indices)
    v_hemo_interp = torch.from_numpy(v_hemo_interp).to(pmd_hemo_with_nn.v.dtype)

    regression_coefficients = per_pixel_lstsq(pmd_gcamp_with_nn.u,
                                              pmd_hemo_with_nn.u,
                                              pmd_gcamp_with_nn.v,
                                              v_hemo_interp,
                                              batch_size=300,
    ...
    blood_basis = compute_blood_basis(pmd_gcamp_with_nn.u,
                                      pmd_gcamp_with_nn.u_local_projector,
                                      pmd_hemo_with_nn.u,
                                      v_hemo_interp,
    ...
    ## Finally, construct a new PMD object describing the various results
    hemo_corr_est = masknmf.PMDArray((pmd_gcamp_with_nn.shape[0], pmd_gcamp_with_nn.shape[1], pmd_gcamp_with_nn.shape[2]),
                                     pmd_gcamp_with_nn.u.cpu(),
                                     v_corr.cpu(),
                                     pmd_gcamp_with_nn.mean_img.cpu(),
                                     pmd_gcamp_with_nn.var_img.cpu(),
                                     u_local_projector=pmd_gcamp_with_nn.u_local_projector.cpu(),
                                     device="cpu",
                                     )
